...
const shikiReady = import('@slidev/client/setup/shiki.ts').then(async (m) => {
  ...
  highlight.value = code => highlighter.codeToHtml(code, { ...defaultHighlightOptions, lang: 'json' })
})
...
const RECORDED_TX = '63E35A5852408699342102FFD08F371DC443973FBB97001AA32728CF69AEBD03'
...
function rhex(n) { return hex(randomBytes(n)) }
function fakeDid() { return `did:key:z6Mk${b58encode(randomBytes(16))}` }
...
const STAGES = [
  ...
  {
    id: 'revoke', label: 'Revoke', triad: 'current', kind: 'normal',
    call: ['POST', '/revoke'],
    narr: 'Now the permission is pulled — compromised, or the job was cancelled. One call.',
  },
  {
    id: 'verify2', label: 'Verify ✕', triad: 'action', kind: 'reject',
    call: ['POST', '/verify'],
...
async function computeReceipts() {
  ...
  ctx.boot = { ...bootstrap, leaf_link_id: leaf, escrow_id: `esc_${leaf.slice(7, 15)}` }
  out.bootstrap = {
  ...
  const artifact = { rows: [[1, 2], [3, 4]], unit: 'kg' }
  ...
  out.attest = {
    ...
    log: { index: 40 + (randomBytes(1)[0] % 9), head: `sha256:${rhex(32).slice(0, 10)}` },
  }

  /* verify #1 — recompute and compare, for real */
  const recheck = await sha256hex(canonical(artifact))
  if (recheck !== ctx.artifactDigest) throw new Error('artifact digest mismatch')
  ctx.revoked = new Set()
  out.verify1 = {
    decision: 'ALLOW',
...
const viewStage = computed(() => (viewIndex.value >= 0 ? STAGES[viewIndex.value] : null))
...
const receiptHtml = computed(() => {
  ...
  return highlight.value ? highlight.value(json) : `<pre class="sd-plain">${esc(json)}</pre>`
})
  ...
  <div class="sever-demo" @click.stop>
    ...
    <div class="sd-timeline">
      <template v-for="(s, i) in STAGES" :key="s.id">
        <div v-if="i > 0" class="sd-connector" :class="{ filled: i <= doneCount - 1 }" />
        <button
          class="sd-bubble"
        ...
        >
          <span class="sd-dot">
            <span v-if="bubbleState(i) === 'done' && s.kind === 'allow'">✓</span>
            <span v-else-if="bubbleState(i) === 'done' && s.kind === 'reject'">✕</span>
    ...
    <div class="sd-controls">
      <button class="sd-run" :disabled="running || doneCount >= STAGES.length" @click="run">
        ▶ Run the loop
      </button>
      <button class="sd-ghost" :disabled="running || doneCount >= STAGES.length" @click="step">
        Step →
      </button>
      <button class="sd-ghost" :disabled="running || doneCount === 0" @click="reset">
        Reset
      </button>
    ...
    <!-- detail panel -->
    <div class="sd-stage" :class="{ empty: !viewStage, reject: viewStage?.kind === 'reject' }">
      <template v-if="viewStage">
        <div class="sd-stage-head">
          <p class="sd-narr">{{ viewStage.narr }}</p>
          <span class="sd-call"><span class="sd-method">{{ viewStage.call[0] }}</span> {{ viewStage.call[1] }}</span>
        </div>
        <div class="sd-resp">
          <div v-if="viewStage.kind === 'allow'" class="sd-verdict allow">✓ ALLOW · escrow released</div>
          <div v-if="viewStage.kind === 'reject'" class="sd-verdict reject">✕ REJECT · settlement blocked</div>
          ...
          <div v-if="viewStage.kind === 'allow'" class="sd-explorer">
            ...
            <a :href="EXPLORER_URL" target="_blank" rel="noopener">testnet.xrpl.org/transactions/{{ RECORDED_TX.slice(0, 16) }}…</a>
          </div>
        </div>
      </template>
      <div v-else class="sd-awaiting">press ▶ Run the loop — every digest below is computed live in this browser</div>
    </div>
  </div>
</template>
...
.sd-connector.filled { background: var(--accent); }
.sd-bubble {
  display: flex;
  flex-direction: column;
  align-items: center;
...
.sd-bubble:disabled { cursor: default; }
.sd-dot {
  width: 52px;
  height: 52px;
  border-radius: 50%;
...
.sd-blabel { font-size: 13.5px; font-weight: 700; color: var(--muted); letter-spacing: 0.4px; }
.sd-btriad {
  font-size: 10px;
  font-weight: 700;
...
.sd-bubble.active .sd-blabel { color: var(--white); }
@keyframes sd-pulse {
...
.sd-bubble.done.allow .sd-blabel { color: var(--green); }
.sd-bubble.done.reject .sd-blabel { color: var(--danger); }
.sd-bubble.done .sd-blabel { color: var(--white); }
.sd-bubble.viewing .sd-dot { box-shadow: 0 0 0 4px rgba(var(--accent-rgb), 0.22); }
...
.sd-run:disabled { opacity: 0.4; cursor: default; }
.sd-ghost {
...
.sd-ghost:hover:not(:disabled) { border-color: var(--accent); }
.sd-ghost:disabled { opacity: 0.4; cursor: default; }
.sd-badge {
  margin-left: auto;
  font-family: var(--mono);
  font-size: 11px;
...
.sd-narr { margin: 0; font-size: 15.5px; color: var(--white); max-width: 980px; }
.sd-call {
  font-family: var(--mono);
  font-size: 12px;
  color: var(--accent);
...
.sd-method { color: var(--danger); font-weight: 700; }
.sd-resp {
  position: relative;
...
.sd-stage.reject .sd-resp { animation: sd-flash 1.1s ease; }
@keyframes sd-flash {
  0% { box-shadow: 0 0 0 0 rgba(255, 138, 97, 0); }
  30% { box-shadow: 0 0 0 4px rgba(255, 138, 97, 0.35); }
  100% { box-shadow: 0 0 0 0 rgba(255, 138, 97, 0); }
}
...
.sd-verdict.allow { background: rgba(112, 214, 168, 0.14); color: var(--green); }
.sd-verdict.reject { background: rgba(255, 138, 97, 0.15); color: var(--danger); }
.sd-explorer { margin-top: 8px; font-size: 12px; color: var(--muted); }
.sd-explorer a { color: var(--accent); font-family: var(--mono); text-decoration: none; }
.sd-explorer a:hover { text-decoration: underline; }
.sd-copy {
  position: absolute;
  top: 8px;
  right: 9px;
...
.sd-copy:hover { background: rgba(255, 255, 255, 0.15); }
</style>
